...
using ConsoleApp1;
{
    ...
    }
    {
        ...
        //Legend(["y", "y'"], UpperLeft, 20, Latex);
        //TickLabelIntepreter(Latex);
    ...
    {
        //HoldOn();
        //var Wall = Rectangle([-0.5, -1.5, 0.5, 3], 0.2);
        //Wall.FillColor = [0.8, 0.8, 0.8];
        //var Block = Rectangle([5, -1, 2, 2], 0.2);
        //Block.FillColor = [0, 0.2, 1];
        //ColVec x = Linspace(0, 5, 21),
        //    y = 0.5 * Sin(2 * pi * x);
        //y[1] = 0; y[^2] = 0;
        ...
        //ColVec Dynamic(double t, ColVec x) =>
        //    new double[] { x[1], -10 * x[0] };
        //byte[] AnimFunc(int i)
        //{
        ...
        //}
        //AnimationMaker(AnimFunc, "Vibration.gif", 30, 130);
        //CloseFig();
    }
    {
        // Damping Systems
    ...
    }
    {
        ...
        {
            if (Pr == 0) return 1;
            double t = 1 / Tr, tm1, tm1e2, A, B, C, D, r, y2, y3, y4, Den;
            ...
            {
                y2 = y * y; y3 = y2 * y; y4 = y3 * y; Den = Pow(1 - y, 3);
                return -A * Pr + (y + y2 + y3 - y4) / Den - B * y2 + C * Pow(y, D);
            });
            r *= Pr < 5 ? 2 : 1; r /= Pr > 13 ? 2 : 1;
            return A * Pr / Fsolve(yfunc, r);
        }

        // set up ressure and temperature mesh
        ColVec Pr = Linspace(0, 15, 501);
        double[] Tr = [1.05, 1.10, 1.15, 1.20, 1.25, 1.30, 1.35, 1.40, 1.45, 1.50,
                       1.60, 1.70, 1.80, 1.90, 2.00, 2.20, 2.40, 2.60, 2.80, 3.00];

        // compute z factors and plot them
        Matrix ZHY = Meshfun(ZfactorHY, Pr, Tr); Figure(640, 880);
        var z1 = Plot(Pr[Pr <= 8], ZHY[Pr <= 8, ..], "k"); GridOn(); HoldOn();
        var z2 = Plot(Pr[Pr >= 7], ZHY[Pr >= 7, ..], "k"); GridOn(); HoldOff();
